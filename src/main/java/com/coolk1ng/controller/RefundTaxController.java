...
public class RefundTaxController {
    ...
    public ResResult getRefundTaxList(@RequestBody RefundTaxDTO refundTaxDTO) {
        LOGGER.info("查询列表参数:{}", JSON.toJSONString(refundTaxDTO));
        return ResResult.success(refundTaxService.getRefundTaxList(refundTaxDTO));
    }

    @PostMapping(value = "/saveRefundTax")
    public ResResult saveRefundTax(@RequestBody RefundTaxDTO refundTaxDTO) {
        LOGGER.info("新增数据参数:{}", JSON.toJSONString(refundTaxDTO));
        refundTaxService.saveRefundTax(refundTaxDTO);
